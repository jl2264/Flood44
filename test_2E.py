...
def test_plot_water_levels():
    ...
    levels2 = [1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 
        1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 
        1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 
        1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 1.00, 
        1.00, 1.00, 1.00, 1.00, 1.00, 1.00]

    test_graph_1 = plot_water_levels(s1, dates, levels) #plot first test graph
    test_graph_2 = plot_water_levels(s2, dates, levels2)    #plot second test graph (which has gradient zero)
    dates_converted = matplotlib.dates.date2num(dates)  #convert dates to floats so they can
                                                        #be worked with as function arguments
    #then calculate gradient and round to avoid any potential floating point error
    gradient_2 = round((levels2[45] - levels2[0])/(dates_converted[45]-dates_converted[0]), 8)
    assert gradient_2 == 0 #graph is horizontal line so gradient should be zero
